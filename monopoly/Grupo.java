    ...
    * Valor devuelto: true si es dueño de todas las casillas del grupo, false en otro caso.
    ...
    public boolean esDuenhoGrupo(Jugador jugador) {
        ...
        for (Casilla miembro : this.miembros) {
            ...
            if(!(miembro instanceof Propiedad propiedad))
                continue;
    ...
    public void mostrarEdificaciones(){
        ...
        for (Casilla casilla : miembros){
            ...
            for (Edificacion edificio : cas.getEdificaciones().getOrDefault("hotel", new ArrayList<>())){
                str.append(edificio.getClass().getSimpleName()).append(" - ").append(edificio.getId()).append(", ");
            }
    ...
    public boolean tieneHipotecaEnGrupo(Jugador jugador){
        for (Casilla casilla : miembros){
            if (!(casilla instanceof Propiedad cas))
                continue;
            if (cas.esHipotecada() && cas.getDuenho()==jugador){
                return true;
            }
        }
        return false;
